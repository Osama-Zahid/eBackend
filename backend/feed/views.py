...
from .serializers import MumbleSerializer,MediaSerializer
...
def create_mumble(request):
    ...
    if is_comment:
        ...
        # fiels = data.get('files')
        # if fiels:
        #     for file in fiels:
        #         serializer = MediaSerializer(file)
        #         if serializer.is_valid(mumble=mumble):
        #             serializer.save()
...
# @api_view(['POST'])
# @permission_classes((IsAuthenticated,))
# def create_mumble_post(request):
#     user = request.user
#     data = request.data
#     mumble = Mumble.objects.create(
#         user=user,
...
#         check_course=Mumble.objects.filter(course_uuid=course_uuid)
